...
export class UserUseCase {
  ...
  */
   ...
   * ```
  */
  public async getUsers(page: number, limit: number): Promise<{ data: DTOUserResponse[], meta: Meta }> {
    ...
    const usersObtained = await this.userRepository.getUsers(meta.getOffset(), meta.getLimit())
    return {
      data: usersObtained.map(user => new DTOUserResponse(user)),
      meta
    }
  }
